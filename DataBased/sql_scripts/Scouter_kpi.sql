...
create view vw_scoring_strength as 
...
-- report to suppor draft
select rank() over (partition by x.event_id order by (TotalPotentialScore * scoreweight) desc) rnk, 
eta.event_team_assoc_id, team_number, scoreweight * 100 scoreweight, 
totalPotentialScore, avg_alliance_links,
round(TotalPotentialScore * scoreweight, 2) as pointContribution,
o3.TripleEngage
from 
(
	select vcs.event_id, t.team_id, t."number" team_number, 
	vas.avg_total_team_score + vcs.defensetelopscore + vcs.cooptelopscore TotalPotentialScore,
	vss.scoreweight/10 scoreweight, vas.avg_alliance_links
	from vw_coop_scores vcs 
	join vw_scoring_strength vss  on vcs.event_team_assoc_id = vss.event_team_assoc_id 
	join public.vw_average_scores vas on vcs.event_team_assoc_id = vas.event_team_assoc_id 
	join teams t on vcs.team_id = t.team_id 
) x
join event_team_assoc eta on x.team_id = eta.team_id and x.event_id = eta.event_id
left join 
(
	select distinct o2.event_id, mta2.team_id, 'Yes' TripleEngage
	from 
	match_team_assoc mta2 
	join teams t2 on mta2.team_id = t2.team_id 
	join
	(
		select m.event_Id, ms.match_id, mm.game_state, mta.alliance_color, count(1) 
		from 
		match_score ms 
		join match_markers mm on ms.match_score_id = mm.match_score_id  
		join teams t on mm.team_id = t.team_id 
		join matches m on ms.match_id = m.match_id 
		join match_team_assoc mta on ms.match_id = mta.match_id and mm.team_id = mta.team_id 
		where marker_type = 'Engaged' and mm.game_state = 'teleop'
		group by m.event_id, ms.match_id, mm.game_state, mta.alliance_color
		having count(1) = 3
	) o2 on o2.match_id = mta2.match_id 
	
) o3 on x.team_Id = o3.team_Id and x.event_Id = o3.event_Id
where team_number = 9080

order by TotalPotentialScore * scoreweight desc



select m.event_Id, ms.match_id, mm.game_state, mta.alliance_color,
mm.marker_x , mm.marker_y,
...
from
(
	select o1.match_score_id ,eta.event_team_assoc_id, m.event_id, o1.team_id, ms.match_id, match_second, sum(point_value) point_value 
	from 
	(
	select match_score_id, team_id, match_second, point_value 
	from public.match_time mt
	...
	where o1.team_id = 10 and m.event_id = 34
	group by o1.match_score_id ,eta.event_team_assoc_id, m.event_id, o1.team_id, ms.match_id, match_second
) o2
group by event_team_assoc_id, event_id, team_id, match_second

select * from match_markers mm 
where mm.team_id = 10
and mm.match_score_id = 140

select *, cast(placed_time as numeric) - (cast(placed_time as numeric)-20) new_placed_time from match_markers mm 
where 1=1
and game_state = 'auton'
--and marker_type in ('Engaged', 'Docked')
and cast(placed_time as numeric) > 130
and mm.match_score_id  in (select match_score_id  from public.match_score ms join matches m2 on ms.match_id = m2.match_id where m2.event_id = 34 )



select * from teams t where t."number" = 5411
select event_team_assoc_id, event_id, team_id, match_second, avg(point_value) point_value
from
(
...
select * from vw_team_comparison vtc 
where event_id =34
